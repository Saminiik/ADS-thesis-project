...
goat_df_GRF_wave$cs_cs_diff_post_goat <- NULL

#Deal with the categorical values
formula <- as.formula("~ .")

# One-hot encoding
X <- model.matrix(formula, data = goat_df_GRF_wave[, setdiff(names(goat_df_GRF_wave), "cs_diff_log")])

X <- X[, -1]

Y <- goat_df_GRF_wave$cs_diff_log
